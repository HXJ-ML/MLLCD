...
def val(model, loader, optimizer, loss_faction, device, grad):
    ...
    for data, target in loader:
        ...
        # for na, pa in model.named_parameters():
        #     pa.retain_grad()
        loss.backward()
        # optimizer.step()
...
def te(model, loader, loss_faction, device, epoch, ith):
    ...
    with torch.no_grad():
        ...
        plt.title('ROC of  5-shots about classifying')
        plt.legend(loc="lower right")
        plt.savefig('/root/autodl-tmp/MLDoLC/training/roc/{}_roc_5shot_{}.png'.format(ith, epoch))
        plt.close()
        return avg_loss, acc
...
HIDDEN_DIM = 600
ITH = 4
print("meta_lr {}, task_lr {}, 5 shots,{} th, patch size{},batch size {}".format(meta_lr, task_lr, ITH,PATCH_SIZE,BATCH_SIZE))
...
for m in range(MATA_EPOCHs):
    ...
    torch.save(obj=net.state_dict(), f="/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_5_{}.pth".format(ITH, m))
    total_grad = {}
    ...
    for i in range(0, len(dataSet_list) - 2):
        train_filename = os.path.join(root, dataSet_list[i], 'SupportSet_5')
        ...
        net.load_state_dict(torch.load("/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_5_{}.pth".format(ITH, m)))
    ...
    torch.save(obj=net.state_dict(), f="/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_5_{}.pth".format(ITH, m + 1))

    # Test
    print("---------------------------------------------TESTING-----------------------------------------------------------")
    net.load_state_dict(torch.load("/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_5_{}.pth".format(ITH, m + 1)))
    test_filename = os.path.join(root, dataSet_list[5], 'SupportSet_5')
