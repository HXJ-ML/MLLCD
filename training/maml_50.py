...
def te(model, loader, loss_faction, device, epoch, ith):
    ...
    with torch.no_grad():
        ...
        plt.title('ROC of  50-shots about classifying')
        plt.legend(loc="lower right")
        plt.savefig('/root/autodl-tmp/MLDoLC/training/roc/{}_roc_50shot_{}.png'.format(ith, epoch))
        plt.close()
        return avg_loss, acc
...
HIDDEN_DIM = 600
ITH = 4
print("meta_lr {}, task_lr {}, 50 shots,{} th,patch size{}".format(meta_lr, task_lr, ITH,PATCH_SIZE))
...
for m in range(MATA_EPOCHs):
    ...
    torch.save(obj=net.state_dict(), f="/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_50_{}.pth".format(ITH, m))
    total_grad = {}
    ...
    for i in range(0, len(dataSet_list) - 1):
        train_filename = os.path.join(root, dataSet_list[i], 'SupportSet_50')
        ...
        net.load_state_dict(torch.load("/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_50_{}.pth".format(ITH, m)))
    ...
    torch.save(obj=net.state_dict(), f="/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_50_{}.pth".format(ITH, m + 1))

    # Test
    print("---------------------------------------------TESTING-----------------------------------------------------------")
    net.load_state_dict(torch.load("/root/autodl-tmp/MLDoLC/pth/{}_meta_parameters_50_{}.pth".format(ITH, m + 1)))
    test_filename = os.path.join(root, dataSet_list[5], 'SupportSet_50')
